...
class LineAnalyzer
  ...
  def calculate_word_frequency(content)
    content = content.to_arr
    calculate most frequent word
    store most frequent word
  end
end
...
class Solution
  ...
  def initialize()
  @analyzers = [] #an array of LineAnalyzer objects for each line in the file
  @highest_count_across_lines = 0 #a number with the maximum value for highest_wf_words attribute in the analyzers array.
  ...
  def analyze_file() #processes 'test.txt' into an array of LineAnalyzers and stores them in analyzers.
    analysers = File.readlines("test.txt")
